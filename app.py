...
st.markdown("""
    ...
    /* עיצוב גיאומטרי: קווים ישרים, ללא פינות מעוגלות, צבעים מדויקים */
    .stButton>button {
...
# ==========================================
# 2. אתחול משתני מערכת וחיבור למודל
# ==========================================
...
*אם חסר נתון מהותי הדרוש להרצת המודל החישובי או למתן הייעוץ, עצור ובקש אותו בנימוס.*
...
* אם התכנית 5 דונם ומעלה (וכוללת בינוי): ציין כי קיימת חובה קטגורית להגיש נספח "ניהול נגר וניקוז" (נספח ב'4) מקיף, מה שמחייב שכירת יועץ ניקוז/הידרולוג לביצוע החישובים. הוסף את ההערה: "היות ששטח התכנית עולה על 5 דונם, חישוב יעד האיגום הזמני ויעד הספיקה היוצאת המופחתת לא מוצג כאן; יעדים אלו יחושבו באופן פרטני לכל תת-אגן במסגרת נספח הניקוז (נספח ב'4) על ידי יועץ הניקוז/ההידרולוג".
...
* למגורים/עירוב שימושים: המלץ על ניהול הנגר בשצ"פים באמצעות פתרונות מבוססי טבע (NBS), ושילוב גגות כחולים/ירוקים. קביעת מפלס 0.00 מעל רום פשט ההצפה (1:100).
* לתשתיות ודרכים: המלץ על ניהול באיי תנועה ושולי דרך, ותכנון מערכת תיעול לעמידה באירועי 1:5 ו-1:20.
* לתעסוקה ומסחר / מוסדות ציבור: המלץ על אגירת מים בתת-הקרקע או ניצול רחבות הריצוף הגדולות להשהיה באמצעות ריצוף מחלחל למחצה. התראה על הפרדת מערכות ניקוז וביוב.

הוראות מסגרת חובה (Knowledge Base & Style):
1. בבואך לפרט על תכולת מסמך "ניהול נגר וניקוז", סעיפי חוק או אמצעי השהיה וחלחול – התבסס אך ורק על "נספח ב'4", תמ"א 1 (תיקונים 7 ו-8), וחוברת העזר לבודקי תכניות המצויים במאגר הידע שלך. אין להמציא סעיפי חוק או תקנות.
2. ודא כי התשובה כתובה בעברית תקינה, ללא שגיאות כתיב, ובהתאם למונחים המקצועיים הנהוגים במינהל התכנון.
3. סגנון: שפה מקצועית, אובייקטיבית וסמכותית. ללא פסקאות הקדמה מאריכות.
4. סיכום: סיים את הייעוץ בפסקת סיכום ברורה המציגה את השורה התחתונה על סמך התחקור והנתונים שהוזנו.
"""
...
with tab1:
    ...
    with col_input:
        ...
        # יצירת מפה אינטראקטיבית בסיסית ממוקדת על ישראל
    ...
    with col_output:
        ...
        if st.button("הפק דוח ייעוץ", use_container_width=True):
            with st.spinner("מנתח נתונים סטטוטוריים..."):
                # יצירת פרומפט דינמי לפי הנתונים (מיוצג כטקסט בסיסי לצורך ההדגמה)
                prompt_data = f"אנא הפק דוח עבור התכנית '{st.session_state['plan_name']}' בשטח של {st.session_state['plan_area']} דונם."
...
with tab2:
    ...
    st.write("מענה לשאלות בודקים, פרשנות תמ\"א 1, מודלים הידרולוגיים והנחיות ניקוז.")
    ...
    if user_chat := st.chat_input("הקלד את שאלתך כאן..."):
        ...
        with st.spinner("מנתח..."):
            res = model.generate_content(user_chat)
            st.chat_message("assistant").markdown(res.text)
            st.session_state["chat_history"].append({"role": "assistant", "content": res.text})

# ------------------------------------------
# כרטיסייה 3: אומדן יעדי נגר (המחשבון)
# ------------------------------------------
with tab3:
    st.subheader("אומדן יעדי נגר ואיגום זמני (לפי מחשבון מינהל התכנון - תיקון 7/8)")
    st.markdown("> *כלי חישוב נפח הנגר לניהול בתכנית ובתכניות בשטח קטן מ 5 ד' גם לחישוב יעד איגום זמני ויעד ספיקה יוצאת מופחתת. המחשבון מתבסס על נתוני קרקע וגשם והמפורסם באתר מינהל התכנון, וכמו כן על-ידי הזנת הפרמטרים הבאים: שטח, מיקום התכנית והתכסית האטומה המוצעת. שינויים בנתונים המצויים בבסיס המחשבון יהיו בסמכות המועצה הארצית, אשר תהיה רשאית לעדכנם מעת לעת, מבלי שהדבר מהווה שינוי לתכנית זו.*")
    ...
    with col_calc_out:
        st.markdown("**תנאי בסיס לכל תכנית:**")
        
        # לוגיקת המחשבון הסטטוטורית כפי שהוגדרה
        if calc_area >= 5.0:
            st.success(f"**תכנית ≥ 5 דונם (חובת 75%)**")
            ...
            st.info(f"**תכנית קטנה מ-5 דונם (חובת 50%)**")
